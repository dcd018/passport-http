 ...
 * @api protected
 */
BasicStrategy.prototype.authenticate = function(req, options) {
  var self = this;
  if (req.body.grant_type == 'client_credentials') {
    self._realm = 'Client';
  }

  this._authInfo = (!!options && !!options.authInfo && Array.isArray(options.authInfo)) ? options.authInfo : [];
  ...
  function verified(err, user) {
    ...
    var info = {};
    self._authInfo.forEach((prop) => {
      if (!!user[prop]) {
        info[prop] = user[prop];
      }
    });

    user = (self._realm == 'Users') ? user : null;
